...
void drawChar(unsigned char x, unsigned char y, char character) {
    // ASCII characters start from 32
    if (character < 32 || character > 127)
        return; // Character out of range

    // Adjust x and y position for the given character
    x = x * 6; // Each character occupies 6 columns (5 + 1 for spacing)
    y = y * 8; // Each character occupies 8 rows

    // Loop through each column of the character
    for (unsigned char col = 0; col < 5; col++) {
        // Get the column bitmap from the font.h
        unsigned char columnBitmap = ASCII[character - 32][col];
        // Loop through each row of the character
        for (unsigned char row = 0; row < 8; row++) {
            // Check if the current bit in the column bitmap is set
            if (columnBitmap & (1 << row)) {
                // Set the corresponding pixel on the screen
                ssd1306_drawPixel(x + col, y + row, 1);
            } else {
                // Clear the corresponding pixel on the screen
...
void drawString(unsigned char x, unsigned char y, char *string) {
    ...
    while (*string != '\0') {
        ...
        drawChar(currentX, currentY, *string);
